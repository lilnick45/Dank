...
namespace Dank.Discord.Labs
{
    class Program
    {
        ...
        static void Main()
        {
            var x = File.ReadAllText(@"UESP WIKI API\Set Details (LARGE FILE).txt");

            var TotalData = JsonConvert.DeserializeAnonymousType(x, new
            {
                minedItemSummary = new[] {
                    new { itemId = default(int),
                        name = default(string)}
                }
            });

            var y = from z in TotalData.minedItemSummary
                    select new UespItem(z.name, z.itemId, UespItemQuality.Default);

            var Data = new UespItemLinkImageDatabase(y);

            var g = Data.FindItems("Iron").FirstOrDefault();
